    ...
    while (p != end_watches) {
        ...
        if (binary) {
            if (blocking_value < 0) {
                ...
                kissat_fast_binary_assign (solver, probing, level, values,
                                           assigned, blocking, not_lit);
                ticks++;
            }
        } else {
            ...
            if (replacement_value >= 0) {
                ...
                assert (lits[0] != lits[1]);
                *r = not_lit;
                kissat_delay_watching_large (solver, delayed, replacement,
                                             other, ref);
                ticks++;
            } else if (other_value) {
...
#endif
                kissat_fast_assign_reference (solver, values, assigned, other,
                                              ref, c);
                ticks++;
            }
        }
